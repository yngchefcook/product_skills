...
path = build_from_spec(spec, output_dir)
print(path, Path(path).exists(), Path(path).stat().st_size)
```
...
- обязательно упомянуть точное имя созданного `.pptx`, чтобы интерфейс смог
  показать файл для скачивания.
